function gbm = f4gbm_setup(n_x, n_y, n_z, n_h, n_f, varargin)
% F3GBMSETUP
%
%
%
...
if isempty(gbm.wzf)
    gbm.wzf = gbm.initMultiplierW*randn(gbm.n_z,gbm.n_f);
end
...
if isequal(gbm.zeromask,'none')
    gbm.zeromask = false((gbm.n_x+gbm.n_y+gbm.n_z+gbm.n_h)*gbm.n_f+gbm.n_y+gbm.n_h, 1);
...
gbm = struct('n_x',[],'n_y',[],'n_z',[],'n_h',256,'n_f',1024, ...
    'sgbmitygain',0.0, ...
    ...
    'everySave',1,'wxf',[],'wyf',[],'wzf',[],'whf',[],'wy',[],'wh',[],...
